...
import { MessageSquare, HelpCircle, ChevronRight, LogOut, Menu, ChevronLeft, Home } from 'lucide-react';
...
export default function MentorLayout({
  ...
  const navItems = [
    {
      label: 'Cố vấn & Đội thi',
      href: '/mentor',
      icon: MessageSquare,
      description: 'Góp ý & phản hồi dự án'
    }
  ];
      ...
      <aside className={cn(
        ...
        <div className="w-80 flex flex-col h-full shrink-0">
          <div className="p-6 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between">
            ...
            <Badge className="bg-sky-50 text-sky-600 border border-sky-100 text-[10px] font-extrabold dark:bg-sky-950/20 dark:text-sky-400 dark:border-sky-900/30">
              MENTOR
            </Badge>
          </div>
          ...
          <nav className="flex-1 px-4 py-6 space-y-1">
            {navItems.map((item) => {
              ...
              return (
                ...
                >
                  {isActive && (
                    <div className="absolute left-0 top-3 bottom-3 w-1 bg-sky-600 rounded-r-full dark:bg-sky-400" />
                  )}
          ...
          <div className="p-6 border-t border-slate-100 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-900/50 flex flex-col gap-4">
            <Link
              href="/"
              className="flex items-center justify-between text-xs text-sky-600 hover:text-sky-700 font-bold dark:text-sky-400 dark:hover:text-sky-300 outline-none w-full text-left cursor-pointer"
            >
              <span className="flex items-center gap-1.5">
                <Home className="w-4 h-4 text-sky-500" /> Quay lại Trang chủ
              </span>
            ...
            <div className="text-[11px] text-slate-400 dark:text-slate-500 leading-normal flex items-start gap-1">
              ...
              <span>Mở rộng kênh trao đổi trực tiếp giúp Mentor hỗ trợ gỡ rối công nghệ cho đội thi nhanh chóng.</span>
            </div>
          </div>
        </div>
      </aside>
