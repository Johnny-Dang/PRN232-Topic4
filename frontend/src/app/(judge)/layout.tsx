...
import { UserCheck, HelpCircle, ChevronRight, LogOut, Menu, ChevronLeft, Home } from 'lucide-react';
...
export default function JudgeLayout({
  ...
  const navItems = [
    {
      label: 'Chấm điểm dự án',
      href: '/judge',
      icon: UserCheck,
      description: 'Phiếu điểm kỹ thuật số'
    }
  ];
      ...
      <aside className={cn(
        ...
        <div className="w-80 flex flex-col h-full shrink-0">
              ...
              return (
                <Link
                  ...
                  className={cn(
                    ...
                    isActive
                      ? "bg-slate-50 text-emerald-600 dark:bg-slate-800/60 dark:text-emerald-400"
                ...
                >
                  {isActive && (
                    <div className="absolute left-0 top-3 bottom-3 w-1 bg-emerald-600 rounded-r-full dark:bg-emerald-400" />
                  )}
          ...
          <div className="p-6 border-t border-slate-100 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-900/50 flex flex-col gap-4">
            <Link
              href="/"
              className="flex items-center justify-between text-xs text-emerald-600 hover:text-emerald-700 font-bold dark:text-emerald-400 dark:hover:text-emerald-300 outline-none w-full text-left cursor-pointer"
            >
              <span className="flex items-center gap-1.5">
                <Home className="w-4 h-4 text-emerald-500" /> Quay lại Trang chủ
              </span>
            ...
            <div className="text-[11px] text-slate-400 dark:text-slate-500 leading-normal flex items-start gap-1">
              ...
              <span>Chấm điểm trực tuyến giúp lưu trữ trực tiếp điểm số chi tiết cho từng tiêu chí, thay thế Excel.</span>
            </div>
          </div>
        </div>
      </aside>
