      ...
      <CardContent className="space-y-6">
        ...
        <form onSubmit={(submitEvent) => void handleSubmit(submitEvent)} className="space-y-4 rounded-2xl border border-indigo-100 bg-indigo-50/40 p-4 dark:border-indigo-900/50 dark:bg-indigo-950/10">
          ...
          <div className="grid gap-3 md:grid-cols-2">
            ...
            <div className="space-y-1.5">
              <label htmlFor="category-name" className="block text-[10px] font-bold uppercase tracking-wide text-slate-500">Tên Category</label>
              <Input id="category-name" required minLength={2} maxLength={150} value={form.CategoryName} onChange={(event) => setForm((current) => ({ ...current, CategoryName: event.target.value }))} placeholder="Ví dụ: Web Application" className="h-10 rounded-xl text-xs" />
            </div>
